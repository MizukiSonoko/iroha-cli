...
import base64
import os
libed2559 = cdll.LoadLibrary(os.path.dirname(__file__)+'/../lib/ed25519/lib/libed25519.so')
...
def generate_hex_sstr():
    ...
    return ("".join([ hex(w).split('x')[1] for w in publist]), "".join([ hex(w).split('x')[1] for w in prilist]))
...
if __name__ == "__main__":
    pub, pri = generate_int()
    print("".join([ hex(w).split('x')[1] for w in pub]))